...
export function BaziOverlayPage({ onClose }: { onClose: () => void }) {
  const [inputMode, setInputMode] = useState<InputMode>('birthProfile');
  const [baziButtonColorIndex, setBaziButtonColorIndex] = useState(() => {
  ...
  return (
    <main className="bazi-demo bazi-page-shell bazi-overlay-page dark relative h-[100dvh] min-h-[100dvh] overflow-hidden">
      ...
      <div className="bazi-fixed-chip bazi-fixed-left absolute z-30">
        <BaziPressableButton
          className="bazi-chip bazi-chip-small bazi-overlay-close-chip"
          onClick={onClose}
          ariaLabel="Close BaZi Atlas overlay"
        >
          <span aria-hidden="true" className="bazi-icon-glyph bazi-close-glyph" />
        </BaziPressableButton>
      </div>
      ...
      <div ref={scrollContainerRef} className="bazi-scroll-hidden relative h-full overflow-y-auto snap-y snap-mandatory">
        ...
        >
          <div className="bazi-section-inner mx-auto flex w-full max-w-4xl flex-col items-center justify-center text-center" style={SECTION_CONTENT_MIN_HEIGHT_STYLE}>
            <a
              aria-label="Download BaZi Atlas on the App Store"
              className="bazi-app-store-button"
              href={appStoreUrl}
              rel="noopener noreferrer"
              target="_blank"
            >
              <span className="bazi-app-store-kicker">Download on the{" "}</span>
              <span className="bazi-app-store-label">App Store</span>
            </a>
          </div>
        </section>
      </div>
